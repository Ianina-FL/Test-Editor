...
module.exports = class CompaniesDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.inventory_companies = await companies.getInventory_companies({
      transaction,
    });

    output.machinery_companies = await companies.getMachinery_companies({
      transaction,
    });

    output.quality_controls_companies =
      await companies.getQuality_controls_companies({
        transaction,
      });

    output.raw_materials_companies = await companies.getRaw_materials_companies(
      {
        transaction,
      },
    );

    output.suppliers_companies = await companies.getSuppliers_companies({
      transaction,
    });

    output.work_orders_companies = await companies.getWork_orders_companies({
      transaction,
    });
